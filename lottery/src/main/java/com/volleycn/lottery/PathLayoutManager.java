...
//            recyclerView.setItemAnimator(null);
    ...
    private void onLayout(RecyclerView.Recycler recycler, List<PosTan> needLayoutItems) {
        ...
        View item;
        float InitAngle = 90;
        float verPanRadius = 360 / needLayoutItems.size();
        float diffRadius = verPanRadius / 2;
        float angle = (needLayoutItems.size() % 4 == 0) ? InitAngle : InitAngle - diffRadius;
...
//            LogUtils.e("__:__"+tmp.getChildAngle());
            if (mScaleRatio != null) {
    ...
    /**
     * 更新Path  平均显示给出在屏幕中的个数
     */
    public void updatePath(Path path, int itemCountInScreen) {
        if (path != null) {
            mKeyframes = new Keyframes(path);
            if (itemCountInScreen == 0) {
                throw new IllegalStateException("itemCountInScreen must be > 0 !!!");
            }
            mItemCountInScreen = itemCountInScreen;
            mItemOffset = mKeyframes.getPathLength() / mItemCountInScreen;
        }
        requestLayout();
    }
